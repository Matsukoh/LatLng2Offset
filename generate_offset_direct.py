...
rect6 = pyproj.Proj(init='EPSG:2444')
...
def latlng_to_diff(latlng_pos):
    ...
    x_origin, y_origin = pyproj.transform(wgs84, rect6, latlng_pos[0][1], latlng_pos[0][0])
    x_ref, y_ref = pyproj.transform(wgs84, rect6, latlng_pos[1][1], latlng_pos[1][0])
    diff_x = x_ref - x_origin
    diff_y = y_ref - y_origin
...
origin_lat = 33.5955422826
origin_lng = 130.2190076182
ref_lat = 33.595433
ref_lng = 130.219152

latlng_pos.append([origin_lat, origin_lng])
latlng_pos.append([ref_lat, ref_lng])
...
with open("offset.txt", 'w') as f:
   ...
   f.write("------------------------------\n")
   f.write("lat, lng\n")
   f.write(str(origin_lat) + "," + str(origin_lng) + "\n")
   f.write(str(ref_lat) + "," + str(ref_lng) + "\n")
